...
**1. Recommended: run the inference engine as an SSH-reachable sidecar, driven by Cookbook like "Local."** `docker/llama-rocm.yml` and `docker/llama-cuda.yml` run a prebuilt, hardware-tuned `llama-server` plus an SSH server trusting Cookbook's own generated key (Cookbook → Settings → SSH key, generate it before first starting the sidecar). Cookbook's existing remote-exec machinery drives it exactly like any other target — GGUF picker, Launch/Stop/Reload, every Advanced flag, tmux-tracked lifecycle. Set `ODYSSEUS_LOCAL_LLAMA_ROCM_HOST=root@llama-rocm` (or `..._CUDA_HOST=root@llama-cuda`) in `.env` and selecting **"Local" + ROCm/CUDA** in Cookbook's Launch screen transparently redirects there instead of running (or failing to run) in the Odysseus container itself — see `model_serve()` in `routes/cookbook_routes.py`. Nothing else Cookbook does (other engines, deps installs, CPU serving) is affected; only that one path redirects.
...
Both sidecars mount the same `huggingface` cache directory Odysseus itself uses, at the same container path — whatever Cookbook downloads locally is immediately visible to the sidecar too, no separate download step. Nothing is pulled/built unless the corresponding file is actually in `COMPOSE_FILE` — no image bloat from having these files present but unused.
