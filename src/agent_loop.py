...
async def stream_agent_loop(
    ...
    def _tool_schemas_for_route(route_state):
        # route_state["mcp_schemas"] is the snapshot taken by the single
        # _build_system_prompt call at turn start. If an MCP server
        # disconnects/reconnects mid-turn, or hadn't finished registering its
        # tools when that snapshot was taken, it stays missing for every
        # remaining round even though tool-rag's separately-cached ToolIndex
        # still selects its tools into relevant_tools. Confirmed incident:
        # retrieval logged a Kanka tool as selected, tools_sent never
        # contained it, and the model flailed calling unrelated tools in a
        # loop. Re-pull live so a mid-turn reconnect shows up on the very
        # next round instead of never. Cheap -- pure in-memory dict walk.
        route_mcp_schemas = (
            mcp_mgr.get_all_openai_schemas(_mcp_disabled_map or {}) if mcp_mgr
            else route_state["mcp_schemas"]
        )
        ...
        if route_state["is_api_model"]:
            if route_relevant_tools:
                ...
                mcp_schema_names = {
                    schema.get("function", {}).get("name") for schema in route_mcp_schemas
                }
                missing_mcp = {
                    t for t in route_relevant_tools if t.startswith("mcp__")
                } - mcp_schema_names
                if missing_mcp:
                    logger.warning(
                        "[tool-rag] round=%s RAG selected MCP tools absent from live "
                        "mcp_schemas (server off for this chat, or disconnected?): %s",
                        round_num, sorted(missing_mcp),
                    )
                mcp_filtered = [
        ...
        # Local models. The keyword check alone only reads the bare last
        # message, which misses ongoing MCP conversations where a short
        # follow-up ("search it to confirm it's deleted") contains none of
        # these words -- confirmed incident: RAG had correctly selected Kanka
        # tools from broader context, but this gate still zeroed the schema
        # list, leaving the model to fabricate a result with no tool at all.
        # relevant_tools already containing an MCP tool is the more
        # context-aware signal, so trust it too.
        wants_mcp = any(keyword in _last_user.lower() for keyword in _MCP_KEYWORDS) or any(
            name.startswith("mcp__") for name in (route_relevant_tools or ())
        )
        if wants_mcp and route_mcp_schemas:
            if route_relevant_tools:
                # Narrow to the RAG selection like the API branch above. A
                # small local model handed every tool from every server at
                # once is far likelier to bleed one tool's parameter
                # convention into another's (a generic "query" where Kanka
                # wants "search_term").
                schemas = [
                    schema for schema in route_mcp_schemas
                    if schema.get("function", {}).get("name") in route_relevant_tools
