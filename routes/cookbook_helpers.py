...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "        # A model's own mmproj/gguf can sit loose directly under the repo dir",
        "        # (e.g. models--org--repo/mmproj-BF16.gguf) instead of inside snapshots/",
        "        # -- manually placed, or written there by a downloader that doesn't use",
        "        # the standard HF snapshot layout. collect_ggufs() above only walks",
        "        # snapshots/<hash>/, so a loose top-level file was never discovered at",
        "        # all: _projectorGgufFiles() found nothing for that model, and a Vision",
        "        # toggle would either warn 'no mmproj found' or, worse, a stale/cached",
        "        # scan from a different model would look like the right answer. Scan",
        "        # the repo dir's own top level (non-recursive; blobs/refs/snapshots/",
        "        # trees are the only subdirs there, so this can't shadow the walk above)",
        "        # and use rel_path='../<file>' so the SAME snapshots/-relative path",
        "        # builder in cookbookServe.js (_selectedGgufExpr) resolves it correctly",
        "        # -- 'snapshots/../mmproj-BF16.gguf' is the repo root, no JS changes",
        "        # needed. Skip filenames already found under snapshots/ so a normal,",
        "        # correctly-laid-out model doesn't get a duplicate entry.",
        "        _dir = os.path.join(cache, d)",
        "        _seen_names = {os.path.basename(gf['rel_path']) for gf in gguf_files}",
        "        if os.path.isdir(_dir) and safe_path(_dir):",
        "            try:",
        "                for _entry in sorted(os.scandir(_dir), key=lambda e: e.name):",
        "                    if not _entry.is_file(follow_symlinks=True): continue",
        "                    if not _entry.name.lower().endswith('.gguf'): continue",
        "                    if _entry.name.startswith('._'): continue",
        "                    if _entry.name in _seen_names: continue",
        "                    _sz = 0",
        "                    try: _sz = _entry.stat().st_size",
        "                    except Exception: pass",
        "                    gguf_files.append({'rel_path': '../' + _entry.name, 'name': _entry.name, 'role': gguf_role(_entry.name), 'quant': gguf_quant(_entry.name), 'size_bytes': _sz})",
        "            except Exception:",
        "                pass",
        ...
        "        # nested one level down (e.g. model_dir=/app/models, and one of its",
        "        # subfolders is really .../HFCache/hub/models--org--repo/...) rather",
        "        # than a single model's own files. Confirmed real incident: exactly",
        "        # this layout made collect_ggufs() below walk EVERY repo under it and",
        "        # merge every model's weights and every model's mmproj into ONE flat",
        "        # gguf_files list for a single fake 'model' named after the folder --",
        "        # so picking Qwen3.8's own weights from that list also pulled in",
        "        # gemma's mmproj (alphabetically first) as 'the' projector, for every",
        "        # model under that folder, regardless of which one was selected.",
        "        # Detect it and delegate to scan_hf() -- which already scopes each",
        "        # models--org--repo correctly -- instead of flattening.",
        "        _nested_hub = None",
        "        for _cand in (fp, os.path.join(fp, 'hub')):",
        "            try:",
        "                if os.path.isdir(_cand) and any(e.startswith('models--') for e in os.listdir(_cand)):",
        "                    _nested_hub = _cand; break",
        "            except Exception:",
        "                pass",
        "        if _nested_hub:",
        "            scan_hf(_nested_hub)",
